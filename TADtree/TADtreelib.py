...
def betadelta(chr,i,j, mats, backgrnd):
	n = j-i
	x,y = [],[]
	for k in range(n):
...
def buildtrees(mat,smat,gmat,bmat,bakmat,t_lim,height,min_size,backgrnd):
	L = smat.shape[0]
...
def getforest(score,height,T_lim,t_lim,min_size,mat):
	L = mat.shape[1]
...
def foresttb(totalscore,traceback_k, traceback_t,start_t,min_size):
...
def use_TADtree(clr, output, M=10, gamma=500):
    ...
    for i,chr in enumerate(chrs):
        ...
        local_parts_array,score = buildtrees(mat,smat,gmat,bmat,bakmat,t_lim,height,min_size,backgrnd)
        print('Assembling TADforest for ' + chr)
        totalscore,traceback_k,traceback_t = getforest(score,height,T_lim,t_lim,min_size,mat)

        if not os.path.exists(output + '/' + chr):
            os.system('mkdir ' + output + '/' + chr)
        duplicates_out = ['\t'.join(['name','proportion_duplicates'])]


        for start_t in range(1,T_lim):
            trees = foresttb(totalscore,traceback_k, traceback_t,start_t,min_size)
            allints = []
            ...
            fname = output + '/' + chr + '/N' + repr(start_t) + '.txt'
            open(fname,'w').write('\n'.join(out))
            
            duplicates_out += ['\t'.join(['N'+repr(start_t), repr(1 - float(len(final_ints)) / len(allints))])]
            
        open(output + '/' + chr + '/proportion_duplicates.txt','w').write('\n'.join(duplicates_out))

    dfs = []
    for chr in chrs:
        pro_dup = pandas.read_csv(output + '/' + chr + '/proportion_duplicates.txt', sep='\t')
        Nbest = pro_dup['name'][0]
        for row in pro_dup:
            if row['proportion_duplicates'] < 0.02:
                Nbest = row['name']
        dfs.append(pandas.read_csv(output + '/' + chr + Nbest + '.txt', sep='\t'))
    df_concat = pandas.concat(dfs).reset_index(drop=True)
    df_concat.to_csv(path_or_buf=os.path.join(output,f'{os.path.basename(clr.filename)}.TADtree'), sep='\t', header=True, index=False)
    return df_concat